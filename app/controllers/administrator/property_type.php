...
public function editproperty()
		...
		{	
			...
			{	
			   ...
			   $updateData['type']  		    = $this->input->post('type');
				  ...
				  $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','property type updated successfully'));
	...
	if($id == 0)	
	{
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select Property type'));
	 redirect_admin('property_type/view_all_property');
		}
	}
	else
	...
	redirect_admin('property_type/view_all_Property');
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Property deleted successfully'));
		redirect_admin('property_type/view_all_Property');
	}

    function addproperty()
  {
  $property = $this->input->post('addproperty'); 
  if(empty($property))
			{
  ...
  if(empty($property))
			...
			$this->Common_model->insertData('property_type',$data);
			 ...
			 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Property type added successfully!'));
			redirect_admin('property_type/view_property');
