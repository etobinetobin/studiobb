...
if (!$this->image_lib->resize()){
   ...
   if($data['cities']->num_rows()==0)
   {
				...
				foreach($pagelist as $res)
				 {
					...
					{
						$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please Delete This City Contained Places First.'));
	                    redirect_admin('neighbourhoods/viewcity');
					}
                    else {
	...
	$city = $this->db->where('id',$id)->get('neigh_city')->row()->city_name;
		...
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Neighborhood City deleted successfully'));
		redirect_admin('neighbourhoods/viewcity');
	}
		...
		else
        {
				 ...
				 // $data7['category'] = $categories;
				  ...
				  if($city_id)
				  {
                   ...
                   $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Place Added Successfully'));
				  redirect_admin('neighbourhoods/viewcity_place');
				  }
                  else
	              {
				...
				foreach($pagelist as $res)
				 {
					...
					if($result->num_rows() != 0)
					{
						$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please Delete This Place Contained Posts First.'));
	                    redirect_admin('neighbourhoods/viewcity_place');
					}
                    else {
			...
			{	
				  ...
				  if($result->num_rows()==0)
				  {
		            ...
		            redirect_admin('neighbourhoods/viewcategory');
					   ...
					   $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Already Used Category'));
...
public function deletecategory()
	...
	$this->load->model('place_model');
				...
				foreach($pagelist as $res)
				 {
					...
					if($result->num_rows() != 0)
					{
						$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please Delete This Category Contained Places First.'));
	                    redirect_admin('neighbourhoods/viewcategory');
					}
                    else {
	                 $this->db->delete('neigh_category',$condition);
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select any Category'));
	 redirect_admin('neighbourhoods/viewcategory');
		}
	}
	else
			...
			{	
               ...
               elseif(!$this->upload->do_upload('big_image3'))
                ...
                if($this->db->insert('neigh_post',$data7))
				{
                 ...
                 $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Neighborhood Place Post Added Successfully'));
...
function viewpost()
{
	...
	$this->load->model('place_model');
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select any Neighborhood Place Post'));
	 redirect_admin('neighbourhoods/viewpost');
		}
	}
	else
    ...
    $this->db->delete('neigh_post',$condition); 
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Neighborhood Place Post deleted successfully'));
		redirect_admin('neighbourhoods/viewpost');
	}
		...
		$config = array(
				  ...
				  $Data['is_featured'] = $this->input->post('is_home');
				   ...
				   $this->db->insert('neigh_photographer',$Data);
				    $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Photographer updated successfully'));
				  redirect_admin('neighbourhoods/viewphotographer');
				  }
                  else
	              {
		         $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','This Photographer Already Used'));
...
public function deletephotographer()
	...
	$this->load->model('place_model');
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select any Photographer'));
	 redirect_admin('neighbourhoods/viewphotographer');
		}
	}
	else
    ...
    $this->db->delete('neigh_photographer',$condition); 
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Photographer deleted successfully'));
		redirect_admin('neighbourhoods/viewphotographer');
	}
	...
	function addtag()
	{
		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run())
			{
				...
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Tag added successfully'));
		            redirect_admin('neighbourhoods/viewtag');
				}
				else 
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Tag not added successfully'));
		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run())
			{
		       ...
		       // {
				...
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Tag updated successfully'));
		            redirect_admin('neighbourhoods/viewtag');
				}
				else 
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Tag not updated successfully'));
		            redirect_admin('neighbourhoods/viewtag');
				}
			//	}
//else {
...
//}
	...
	//	$getplace	 =	$this->place_model->getplace();
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select any Tag'));
	 redirect_admin('neighbourhoods/viewtag');
		}
	}
	else
	...
	$condition = array('id'=>$id);
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Tag deleted successfully'));
		redirect_admin('neighbourhoods/viewtag');
	}
...
function addknowledge()
	{
		...
		if($this->input->post('submit'))
		{
			...
			$this->form_validation->set_rules('city','city','required|check_city');	
			$this->form_validation->set_rules('place','place','required|check_city');	
			...
			if($this->form_validation->run())
			{
				...
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Knowledge added successfully'));
		            redirect_admin('neighbourhoods/viewknowledge');
				}
				else 
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Knowledge not added successfully'));
	...
	$id = is_numeric($this->uri->segment(4))?$this->uri->segment(4):0;
		...
		if($this->input->post('submit'))
		{
			...
			if($this->form_validation->run())
			{
				...
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Knowledge updated successfully'));
		            redirect_admin('neighbourhoods/viewknowledge');
				}
				else 
				{
					$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Knowledge not updated successfully'));
		            redirect_admin('neighbourhoods/viewknowledge');
				}
		}
		}
...
public function deleteknowledge()
	...
	if($id == 0)	
	{
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select any local knowledge'));
	 redirect_admin('neighbourhoods/viewknowledge');
		}
	}
	else
	...
	$condition = array('id'=>$id);
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Local Knowledge deleted successfully'));
		redirect_admin('neighbourhoods/viewknowledge');
	}
	}
	?>
