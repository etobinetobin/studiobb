...
$this->path=realpath(APPPATH);
	...
	{	
		...
		{	
			...
			if($this->form_validation->run())
			{
				  ...
				  $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Help added successfully'));
	...
	if($id == 0)	
	{
		...
		{
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('error','Please select help'));
	 redirect_admin('help/viewhelp');
		}
	}
	else
	...
	$this->help_model->deletehelp(NULL,$condition);
		...
		//Notification message
		$this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Help deleted successfully'));
		redirect_admin('help/viewhelp');
	}
	//Function end
			...
			{	
			   ...
			   $updateData['description']  		  = $this->input->post('description');
				...
				$updateData['status']  		     = $this->input->post('status');
				  ...
				  //Notification message
				  $this->session->set_flashdata('flash_message', $this->Common_model->admin_flash_message('success','Help updated successfully'));
				  redirect_admin('help/viewhelp');
		 	} 
		} //If - Form Submission End
