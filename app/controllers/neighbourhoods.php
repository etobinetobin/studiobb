...
function suggest_tag()
{
	...
	if($this->dx_auth->get_user_id() )
    {
		...
		$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success','Thanks for your tag suggestion! You might see it on the site soon.'));
...
function local_knowledge()
{
	...
	if($this->dx_auth->get_user_id() )
    {
    	$data = array('city_id'=>$city_id,'city'=>$city,'place_id'=>$place_id,'place'=>$place,'user_id'=>$this->dx_auth->get_user_id(),'knowledge'=>$knowledge);

		$this->db->insert('neigh_knowledge',$data);
		
		$this->session->set_flashdata('flash_message', $this->Common_model->flash_message('success','Thank you so much for contributing to this page! We do our best to build these community pages out from contributions like yours.'));
		redirect('neighbourhoods/city_detail/'.$city1.'/'.$place1);
