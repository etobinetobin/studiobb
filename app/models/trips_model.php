	  ...
	  function Trips_model() 
	  {
			...
			function get_reservation($conditions = array(), $limit = array(), $conditions_or = array())
			{
				...
				if(count($conditions_or) > 0)		
					...
					$this->db->select('reservation.id,reservation.list_id,reservation.userby,reservation.userto,reservation.checkin,reservation.checkout,reservation.no_quest,reservation.currency,reservation.price,reservation.topay ,reservation.admin_commission ,reservation.credit_type,reservation.ref_amount,reservation.status,reservation.book_date,reservation.is_payed,reservation_status.name');
