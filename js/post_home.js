...
var PostRoom = {
	...
	resetLocation: function(currentResult) {
		...
		$("#post_room_submit_button").hide();
		//$("#sublet").removeAttr("checked").change();
	...
	validateSubmit: function() {
		...
		if (phone.is(':visible') && !Cogzidel.StringValidator.validate('phone', phone.val())) {
   PostRoom.addError(Translations.phone_number, Translations.phone_number_error, 'hosting_phone'); 
 }
		...
		if (priceNative === '') { 
			...
			PostRoom.addError(Translations.price, Translations.priceTooSmall_error+' '+curr, "hosting_price_native");
		}
